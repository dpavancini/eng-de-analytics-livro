...
Nesta seção, vamos apresentar como fazer a modelagem de um DW na prática usando o dbt. Para tanto, precisamos primeiro realizar a carga dos dados brutos em nosso DW [(que fizemos na seção anterior)](). Com os dados já inseridos no DW, vamos criar uma série de modelos para materializar o modelo conceitual do DW que criamos [anteriormente]().

Antes de começar a parte técnica, é bom lembrar que o objetivo de um DW é gerar resultados para a empresa, e não criar um sistema que, embora possa ser tecnicamente bem feito, não tenha uma visão de negócio. Desse modo, a forma como criamos os marts e suas tabelas fato e dimensões deve ser sempre pensada com base nos problemas prioritários para tomada de decisão e na utilização pelo usuário final. Um erro comum de projetos de DW é pensar mais nos modelos de dados das aplicações-fonte, e menos no modelo de negócios do usuário final. Se for difícil saber *a priori* como será essa utilização, podemos recorrer a instrumentos já usados na tomada de decisão, como relatórios, planilhas, etc. para ter uma ideia de quais relações e atributos são mais relevantes para o cliente.

Atualmente, o [dbt](https://www.getdbt.com/) possui uma versão SaaS chamada [dbt cloud](https://www.getdbt.com/product/what-is-dbt/), além da versão *open-source*. A versão SaaS possui uma curva de aprendizagem menor para quem tem menos contato com programação, mas neste curso vamos utilizar a versão *open-source*. No geral, ambas são muito similares e você poderá utilizar a versão cloud de forma equivalente, se desejar.

```{admonition} Dica!
Para lembrar os comandos do dbt facilmente, utilize esta [cheatsheet](https://medium.com/indiciumtech/17-dbt-commands-you-should-start-using-today-581998dbf8f0).
```
...
O primeiro passo de um projeto dbt é a criação da pasta onde ficarão estruturados os arquivos do projeto. A forma mais fácil de criar essa pasta é por meio do comando `dbt init`. Você também vai precisar criar um repositório git na pasta para controlar o versionamento do projeto. Todo projeto dbt é iniciado com um arquivo `dbt_project.yml` padrão, onde são configuradas diversas opções do projeto, incluindo a conexão com o *data warehouse*, o tipo de materialização das tabelas, entre outros.

```{admonition} Atenção!
Se você clonou o repositório de exemplo, as etapas desta subseção já terão sido feitas para você.
```
...
A conexão com o *data warehouse* é configurada por padrão no arquivo `profiles.yml`, que fica armazenado no caminho `~/.dbt/profiles.yml` (LINUX). As permissões do arquivo devem ser restritas ao usuário, pois contém credenciais de acesso aos dados que não devem ser compartilhadas.

A opção `target` permite alternar entre um ambiente de desenvolvimento e um de produção de forma simples. O target de produção deve se chamar “prod”, pois é usado em outras APIs do dbt. Durante o desenvolvimento, esse target não deve ser usado, mas sim um target “dev” ou equivalente.
...
Para verificar se tudo está bem configurado, podemos usar o comando `dbt debug` no terminal. Em caso de sucesso, devemos ter um output como este:
...
Se você não tem familiaridade com Git, recomendamos revisar o capítulo sobre {ref}`Git<GIT>` ou buscar outros conteúdos *online*.
```
...
Agora que configuramos nossa conexão com o *data warehouse*, precisamos criar nossas primeiras fontes de dados. Essas fontes são definidas no arquivo `sources.yml`, onde é possível descrever quais bancos de dados, schemas e tabelas queremos deixar disponíveis no dbt.

```{admonition} Atenção!
Nesta seção, é esperado que os dados brutos da Northwind já estejam disponíveis no *data warehouse* através de um processo de ingestão como visto {ref}`aqui<ingestao_pratica>` ou por meio de um dbt seed como sugerido [aqui](https://github.com/engdeanalytics/northwind).
```
...
Uma boa prática é incluir o arquivo `sources.yml` no diretório *staging*, onde fazemos as primeiras transformações sobre os dados originais. Em geral, **cada source deve ser referenciada apenas em um modelo**, facilitando a consistência na modelagem, como discutiremos na próxima seção.
...
Nos modelos *staging* (abreviados com o prefixo `stg`), importamos as *sources* e aplicamos transformações simples para “limpar” os dados da origem e adequá-los ao nosso modelo de dados:
- Renomear colunas
...
Para facilitar nosso entendimento, também criamos um modelo conceitual com o mapeamento das relações. Na prática, podemos fazer isso diretamente no código e depois deixar o próprio dbt gerar este diagrama para nós:
...
A partir deste diagrama, podemos definir uma sequência de tarefas necessárias para modelar nosso data mart antes de rodar o pipeline:

1) Incluir todas as tabelas necessárias no arquivo `sources.yml`
2) Criar os modelos *staging* necessários
3) Criar as tabelas dimensão e as respectivas chaves surrogate
...
Vamos omitir as etapas 1 e 2 e partir diretamente para a etapa 3: criar as tabelas dimensão e as chaves surrogate. Vamos iniciar pela tabela de clientes (*customers*) e criar uma chave surrogate autoincremental a partir da chave de negócio `customer_id`:

```{admonition} Importante!

As chaves autoincrementais são muito utilizadas em bancos de dados e de fácil entendimento pelos consumidores dos dados. No entanto, em *data warehouses* modernos elas podem ser perigosas. Por quê?
```
...
models:
  northwind:
      ...
      marts:
          materialized: table
```
**Exemplo de configuração de materialização no dbt_project.yml**

Em geral, tabelas são preferíveis para otimizar as consultas no BI, pois possuem um melhor desempenho de leitura que uma visualização (*view*). Durante a etapa de desenvolvimento, o uso de *views* geralmente é mais rápido na hora de executar o pipeline, principalmente se as tabelas forem muito grandes.

## Escrevendo testes

Garantir a integridade dos dados na transformação é parte essencial de qualquer projeto de ETL, e o dbt foi pensado com as melhores práticas de engenharia de software em mente, em especial a capacidade de escrever testes sobre os dados de forma direta e consistente.
...
- **testes genéricos**: são definidos em um arquivo `.yml` e permitem realizar testes gerais sobre colunas da tabela, tais como: unicidade, não-nulidade, valores dentro de uma lista predefinida, relacionamentos, entre outros. O teste falha quando há algum registro que não passa em um teste específico. Por exemplo, se houver um registro duplicado em uma coluna com teste de unicidade.
...
De forma geral, devemos incluir ao menos um teste genérico para cada modelo em sua chave primária (seja ela surrogate ou natural). Por exemplo, para garantir que não tenhamos nenhum cliente repetido na tabela de clientes, vamos criar um arquivo `dim_customers.yml` e incluir dois testes para a chave sk:
...
É comum que durante o processo de modelagem ocorram transformações que porventura dupliquem registros que deveriam ser únicos (um join equivocado, por exemplo) ou que haja regras de negócio que não conhecíamos de antemão. Para esses casos, escrever testes de forma consistente facilita muito o trabalho de validação e o desenvolvimento dos modelos com segurança. Além de unicidade e não nulidade, outros testes de schema comuns a serem incluídos nos modelos são:

- **Teste de relacionamento (relationship)**: os testes de relacionamento servem para garantir a integridade dos relacionamentos entre colunas de modelos distintos, de forma similar ao comportamento de chaves estrangeiras em um banco transacional. Geralmente, são utilizados para mapear chaves de dimensões dentro das tabelas fato.
...
- **Teste de valores aceitos (accepted_values)**: esse tipo de teste serve para garantir que os valores de uma coluna pertençam a um intervalo predefinido, como, por exemplo, em uma coluna de status de pedido.
...
Na maioria dos casos, a grande dificuldade no processo de modelagem de dados é garantir que indicadores e resultados sejam os mesmos (ou próximos) dos considerados ‘verdadeiros’ pelo cliente. Não raramente o próprio cliente não tem uma visão clara dos passos necessários, das fontes de dados e das regras de negócio que são aplicadas antes da geração de um indicador em um relatório ou planilha de Excel. É o trabalho do analytics engineer trabalhar de forma integrada com o cliente para extrair essas informações e aplicá-las no processo de modelagem. Para facilitar esse trabalho, podemos utilizar os testes de dados, isto é, consultas SQL arbitrárias em cima dos modelos que garantem que eles cheguem ao resultado desejado. Para evitar outros problemas, como dados mutáveis e atrasos no ETL, o ideal é sempre fixarmos um período retroativo no qual tenhamos confiança de que os dados não irão se alterar.
...
A última etapa dentro de um projeto padrão de dbt é a documentação dos modelos e transformações. Essa documentação é feita a partir dos arquivos `schema.yml` que já utilizamos para escrever nossos testes de schema através de campos de descrição de tabelas e colunas. Ao criarmos as descrições, elas são adicionadas a outras informações que o dbt processa sobre como as fontes, os modelos e os testes do projeto estão estruturados. No exemplo abaixo, documentamos a tabela dimensão Produtos e cada uma de suas colunas, de preferência na mesma ordem que no modelo final:
...
Embora a documentação no código já seja importante para o entendimento dos modelos, é ao utilizarmos o comando `dbt docs` que vemos o diferencial do dbt nesse quesito. O comando `dbt docs generate` gera um arquivo de documentação em formato HTML que podemos visualizar de forma interativa no navegador, mesmo em projetos complexos, facilitando a colaboração e o compartilhamento da modelagem. A visualização é feita por meio do comando `dbt docs serve` no terminal:
